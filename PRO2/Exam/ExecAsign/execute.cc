...
int evaluate(map<string,int> &var2val, BinTree<string> t){
    if (t.value()=="+"){
        int a = evaluate(var2val, t.left()) + evaluate(var2val,t.right());
        while (a>=10)
        {
            a = a%10;
        }
    ...
    else if(t.value()[0] == 'x') return var2val[t.value()];
    else return t.value()[0] - '0';
}
...
void execute(map<string,int> &var2val, BinTree<string> t){
    ...
    else if(t.value()=="=") var2val[t.left().value()] = evaluate(var2val, t.right());
}
