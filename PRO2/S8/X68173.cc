...
void Cjt_estudiants::afegir_estudiant(const Estudiant& est) {
    ...
    if(imax==i+1) recalcular_posicio_imax();
        ...
        {
            if (vest[i+1].consultar_DNI()<vest[imax].consultar_DNI())
            {
                imax=i+1;
            }
    ...
    else if(vest[i+1].te_nota() and imax==-1) imax=i+1;
}
...
void Cjt_estudiants::recalcular_posicio_imax(){
    ...
    {
        if (vest[i].te_nota() and imax!=-1)
        {
            if (vest[i].consultar_nota()>vest[imax].consultar_nota())
            {
                imax=i;
            }
